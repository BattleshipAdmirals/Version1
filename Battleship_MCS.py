...
class Battleship_MCS:
    ...
    def give_guess(self, heatmap):
        ...
        if not x_tiles: # If no 'X' tiles, default to the highest heatmap value
        ...
        max_val = max(heatmap[tile[0]][tile[1]] for tile in valid_guesses) # Select the highest heatmap value among valid guesses
        ...
        return random.choice(highest_list) # Choose a random tile from the highest values
    ...
    def generate_dummy(self):
        ...
        if len(ships_to_place) == previous_ships_count:
            print("No progress made in ship placement. Exiting.") #if something breaks, gives error
            return dummy_board
    ...
    def place_ships_on_hits(self, board, x_list, ships_to_place):
        while x_list and ships_to_place:
            ...
            for ship_size in ships_to_place[:]:  # Iterate over ships to place
                ...
                if not valid_ship_found:
                    ships_to_place.remove(ship_size)
            # Exit if no progress was made in this iteration
